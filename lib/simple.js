...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/5a49e56d835f691519140.jpg')
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), forwardingScore: fsizedoc, externalAdReply: { body: wm, containsAutoReply: true, mediaType: 1, mediaUrl: sig, showAdAttribution: true, sourceId: null, sourceType: 'PDF', previewType: 'PDF', sourceUrl: null, thumbnail: data, thumbnailUrl: sig, title: namebot }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)

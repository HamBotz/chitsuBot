...
const authFile = `${opts._[0] || 'Chitsu'}`
...
function purgeSessionSB() {
    ...
    const listaDirectorios = fs.readdirSync('./jadibot/');

    let SBprekey = [];

    listaDirectorios.forEach((filesInDir) => {
        const directorio = fs.readdirSync(`./jadibot/${filesInDir}`);
        ...
        DSBPreKeys.forEach((fileInDir) => {
            fs.unlinkSync(`./jadibot/${filesInDir}/${fileInDir}`);
        });
    });
}
...
function purgeOldFiles() {
    ...
    const directories = ['./features/', './jadibot/'];
...
setInterval(() => {
  fs.readdir('Chitsu', (err, files) => {
    if (err) {
      console.log(err);
    } else {
      files.forEach((file) => {
        if (file.startsWith('pre-key')) {
          fs.unlink(`Chitsu/${file}`, (error) => {
            if (error) {
              console.log(error);
            } else {
...
async function connectionUpdate(update) {
	...
	// console.log(JSON.stringify(update, null, 4))
	 ...
	 if((usePairingCode || useMobile) && fs.existsSync('./Chitsu/creds.json') && !conn.authState.creds.registered) {

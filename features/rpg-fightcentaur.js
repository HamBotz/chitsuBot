let handler = async (m, { conn, participants }) => {
  ...
  if (
    ...
    return m.reply(
      `Tidak bisa melakukan battle ⚔️ karena arena yang kamu miliki dipakai untuk fight pet mu yang lain.`,
    );
  }
  ...
  m.reply(
    `Pet🐴 Kamu (centaur ${global.db.data.users[m.sender].centaur}) menantang 🐴centaurnya ${conn.getName(lawan)} (centaur ${global.db.data.users[lawan].centaur}) lagi berkelahi.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yang menang🎮.`,
  );
